...
%for j=1:1:10,
B=0;  %calculated from radiation
    
top=dlmread('top');
pa=0;%dlmread('pa1');%pa1 =200nmperovd, 2=240nm, 3=1000nm
tr=0;%dlmread('tr1');
...
str1= ['Gtop' num2str(top(k)) '.gen'];    
pc1dSet('perov.prm','CExcite::m_Filename',str1);

str2= ['Gbot' num2str(top(k)) '.gen'];   
...
dis=1:1:top(k);
    dis=dis';
     LogDis=logspace(log10(dis(1)),log10(top(k)-0.00001),100);%100 used as universal number of pc1d input
...
if a<=Jscb,
    ...
    power=Jscb*(interp1(data1(10:60,1),data1(10:60,2),Jscb,'pchip'))*(Jscb/a)^2;
else
    x2=min(find(data1(:,2)>0.2)); y2=min(find(data2(:,1)>0));
    Vb =interp1(data2(x2:y2,1),data2(x2:y2,2),a,'pchip');
    power=a*(Vb +V1);
end
