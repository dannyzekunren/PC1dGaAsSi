...
function TransferMatrixtandem
%------------BEGIN USER INPUT PARAMETERS SPECITIFCATION---------------
%
lambda=300:1200; % Wavelengths over which field patterns are calculated
...
thicknesses = [0 50 90 30 top 50 120 3000 1000 120 2000 100 ];  % thickness of each corresponding layer in nm (thickness of the first layer is irrelivant)
active=5;
...
plotGeneration = true;
 % index of material layer where photocurrent is generated
%
...
if plotGeneration == true
    ...
    AM15_data=xlsread('AM15.xls');%%% for singapore
    AM15=interp1(AM15_data(:,1), AM15_data(:,2), lambda);tG = thicknesses(1,active-2:active+1);tG(1)=0; cumtG=cumsum(tG);G=zeros(max(cumtG),1);
     for j=1:3, activeLayer=active-2+j;
    ...
    dis=1:1:max(cumtG);
    dis=dis'/1000;
     LogDis=logspace(log10(dis(1)),log10((max(cumtG)/1000-0.00001)),100);
    CumGen=interp1(dis,cumsum(G),LogDis);
    
    Gcell=[LogDis;CumGen/1e7];
    ...
    assignin('base','Cumgen',CumGen);
    assignin('base','Gcell',Gcell');
    Jsc=sum(Gx)*stepsize*1e-7*q*1e3 ;%in mA/cm^2
 
    str= ['Gtop' num2str(top) '.gen']
   
    dlmwrite(str, Gcell,'delimiter', '\t','newline','pc');
    end
